...
%     idx2=1;
    ...
    %----------------------------------------------------------------------
    Ref1=Frame_ODD_Pre_double;
    Ref2=Frame_ODD_Next_double;
...
%     title(['Reconstruct ' int2str(i) ' at ENCODER']); 
    ...
    temp2 = calculate_psnr(currentFrameY,reconstructedMatrix);
    temp4 = calculate_psnr(currentFrameY,image8Bit_Side_Information);
 
    dec_psnr = [dec_psnr,temp2];
    SI_psnr  = [SI_psnr,temp4];
    
    imwrite(currentFrameY,'.\Transfered\WZFrame.jpg')
    fileInfo                  = dir('.\Transfered\WZFrame.jpg');
    bytesTransfered           = [fileInfo.bytes];
    bitsTransfered            = bytesTransfered*8;
    bits_WZ                   = [bits_WZ,bitsTransfered];    
