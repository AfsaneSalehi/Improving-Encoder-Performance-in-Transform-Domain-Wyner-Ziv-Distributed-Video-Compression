...
for i = 2:GOP_len:(frame_num-1)
    ...
    %----------------------------------------------------------------------
    % 1 % MCE motion compensation extrapolation
    Ref1=Ceq_r1;
    Ref2=Ceq_r2;
    %frame_extrap = MCE(Ref1,Ref2,block_size);
    %method = SI_methods{1};
    % 2 % AR_Fusion auto regressive
    %[frame1,w1] = MCE(Ref1,Ref2,block_size);
    %[frame2,w2] = AR_extrap(Ref1,Ref2,block_size,L);
    %[frame3,w3] = Backward_AR_extrap(Ref1,Ref2,block_size,L);
    %frame_extrap = prob_fusion3(frame1,w1,frame2,w2,frame3,w3);
    %method = SI_methods{2};
    % 3 % MAAR_avg motion aligned auto regressive
    frame1 = MAAR_extrap(Ref1,Ref2,block_size,L,overlap_size);
    frame2 = Backward_MAAR_extrap(Ref1,Ref2,block_size,L,overlap_size);
    frame_extrap = (frame1+frame2)/2;
  
    Side_Information_Double=frame_extrap;
