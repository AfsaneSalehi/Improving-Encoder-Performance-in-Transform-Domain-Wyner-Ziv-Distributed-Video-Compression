...
for i = 2:GOP_len:(frame_num-1)
    ...
    %% SI Interpolation.3 Methods{MCE,AR,MAAR}
    %----------------------------------------------------------------------
    Ref1=Ceq_r1;
    Ref2=Ceq_r2;
    im_prev_pad = padarray(Ref1,params.search_range*[1,1],'replicate');
    im_next_pad = padarray(Ref2,params.search_range*[1,1],'replicate');
...
%   Side_Information_Double=double(Side_Information(:,:,i));
    ...
    title(['Side_Information ' int2str(i) ' at ENCODER']);
    ...
    %% Quantization of SI. 'Uniform_Q','DPCM_Q','PDSQ_Q','ST_Q'
    %----------------------------------------------------------------------
    %SI_Quantized = Uniform_Q(SI_DCTCurrentFrame,level_QP);
    %SI_Quantized             = DPCM_Q(SI_DCTCurrentFrame,level_QP);
	%[SI_Quantized,q]        = MFPQ_Q(SI_DCTCurrentFrame,level_QP,blk_rows);
    [SI_Quantized,q,y_index]= PDSQ_Q(SI_DCTCurrentFrame,level_QP,blk_rows);
...
%     title(['Y Component of Difference frame for Current Frame No. ' int2str(i) ' at ENCODER']);
    ...
    %% Quantization of WZ. 'Uniform_Q','DPCM_Q','PDSQ_Q','ST_Q'
    %----------------------------------------------------------------------
    %WZQuantized = Uniform_Q(YDCTCurrentFrame,level_QP);
    %WZQuantized = DPCM_Q(YDCTCurrentFrame,level_QP);
	%[WZQuantized,q] = MFPQ_Q(YDCTCurrentFrame,level_QP,blk_rows);
    [WZQuantized,q,y_index] = PDSQ_Q(YDCTCurrentFrame,level_QP,blk_rows);
	...
	%----------------------------------------------------------------------    
    %y_hatWZ  = Uniform_Qinv(inverseZigZagCurrentY,level_QP);
    %y_hatWZ   = DPCM_Qinv(inverseZigZagCurrentY,level_QP);
    %y_hatWZ  = MFPQ_Qinv(inverseZigZagCurrentY,q,blk_rows) 
    y_hatWZ  = PDSQ_Qinv(inverseZigZagCurrentY,q,y_index,blk_rows);
    ...
    total_psnr                = mean(total_psnr+temp2);
    RD_WZ_total               = (frame_rate/2)*mean(total_bits)/1000;
